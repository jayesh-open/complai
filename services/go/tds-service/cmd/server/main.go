...
func main() {
	...
	pool, err := db.NewPool(context.Background(), envOr("DATABASE_URL", "postgres://complai:complai_dev@localhost:5432/tds_db?sslmode=disable"))
	if err != nil {
	...
	go func() {
		log.Info().Str("port", port).Msg("tds-service listening")
	...
	log.Info().Msg("tds-service stopped")
}
