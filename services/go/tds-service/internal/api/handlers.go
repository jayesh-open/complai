...
func (h *Handlers) CalculateTDS(w http.ResponseWriter, r *http.Request) {
	var req struct {
		PaymentCode    domain.PaymentCode `json:"payment_code"`
	...
	if !domain.ValidPaymentCode(req.PaymentCode) {
		writeError(w, http.StatusBadRequest, "invalid payment_code")
		return
	}
...
func (h *Handlers) CreateEntry(w http.ResponseWriter, r *http.Request) {
	...
	paymentCode := domain.PaymentCode(req.PaymentCode)
	if !domain.ValidPaymentCode(paymentCode) {
		writeError(w, http.StatusBadRequest, "invalid payment_code")
		return
	}
	...
	section := domain.SectionForCode(paymentCode)
	...
	agg, _ := h.store.GetAggregate(r.Context(), tenantID, deducteeID, paymentCode, req.FinancialYear)
	...
	entry := &domain.TDSEntry{
		...
		Section:          section,
		PaymentCode:      paymentCode,
		SubClause:        domain.SubClauseForCode(paymentCode),
		FinancialYear:    req.FinancialYear,
		TaxYear:          domain.TaxYearFromFY(req.FinancialYear),
		Quarter:          req.Quarter,
